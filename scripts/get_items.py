...
def process_heat_connections(data, key2):
    ...
    for heat_source in ["energy_source", "heat_buffer"]:
        if heat_source in data and isinstance(data[heat_source], dict) and "connections" in data[heat_source]:
...
def generate_js_output(sorted_items, pipe_positions, heat_positions, fluid_recipes, tile_layers):
    ...
    js_output.extend(
        [
            ...
            *[f'    "{item}": {pos!s},' for item, pos in sorted(pipe_positions.items())],
            "}",
        ]
    )
    ...
    js_output.extend(
        [
            ...
            *[f'    "{item}": {pos!s},' for item, pos in sorted(heat_positions.items())],
            "}",
        ]
    )
...
def main():
    ...
    for key1, data1 in factorio_json.items():
        if isinstance(data1, dict):
            for key2, data2 in data1.items():
                ...
                items[key2].update(
                    {"collision_box": data2["collision_box"], "selection_box": data2["selection_box"]}
                )
    ...
    js_output = generate_js_output(
        sorted_items, item_to_pipe_target_positions, item_to_heat_target_positions, fluid_recipes, tile_layers
    )
