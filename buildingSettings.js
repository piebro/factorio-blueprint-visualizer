...
const entityNameToProperties = {
    ...
    "storage-tank": { size: [3,3], genericTerms: [] },
    ...
    "straight-rail": { size: [2,2], genericTerms: ["rails", "transportation"] },
    "curved-rail": { size: [2,2], genericTerms: ["rails", "transportation"] },
    ...
    "heat-exchanger": { size: [3,2], genericTerms: ["nuclear"] },
    "steam-turbine": { size: [3,5], genericTerms: ["electricity-generators"] },
    ...
    "pumpjack": { size: [3,3], genericTerms: ["drills"] },
    ...
    "flamethrower-turret": { size: [2,3], genericTerms: ["military", "turret"] },
    "artillery-turret": { size: [3,3], genericTerms: ["military", "turret"] },
    "radar": { size: [3,3], genericTerms: ["military"] },
};

// Generate BUILDING_SIZES from entityNameToProperties
const BUILDING_SIZES = Object.fromEntries(
    Object.entries(entityNameToProperties)
        .filter(([_, props]) => props.size)
        .map(([name, props]) => [name, props.size])
);

// Generate BUILDING_PIPE_CONNECTIONS from entityNameToProperties
const BUILDING_PIPE_CONNECTIONS = Object.fromEntries(
    Object.entries(entityNameToProperties)
        .filter(([_, props]) => props.pipeConnection)
        .map(([name, props]) => [name, props.pipeConnection])
);
